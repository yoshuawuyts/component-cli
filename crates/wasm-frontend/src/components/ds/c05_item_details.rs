...
pub(crate) fn render() -> &'static str {
    r##"
    <section id="c-item-details" class="pt-12 md:pt-16">
      <div class="grid md:grid-cols-[200px_1fr] gap-6 md:gap-12">
        <div>
          <div class="text-[12px] mono uppercase tracking-wider text-ink-500">C05</div>
          <h2 class="mt-2 text-[24px] font-semibold tracking-tight">Item Details</h2>
          <p class="mt-2 text-[13px] text-ink-500 leading-relaxed">
            Reference page for a single endpoint, RPC, schema, or command. A
            method/kind pill anchors the symbol below the title; a one-sentence
            tagline explains it; an optional structured request-body table, a
            responses list, and paired example panels stack below in fixed order.
            Used as the destination from <a href="#c-item-list"
              class="text-ink-700 underline decoration-line decoration-1 underline-offset-[3px] hover:text-ink-900">Item
              List</a> rows.
          </p>
        </div>
        <div class="space-y-8">
